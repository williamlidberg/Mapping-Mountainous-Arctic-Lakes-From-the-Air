...
class watersheds:
    ...
    def unnest_basins(self, f, d8_pointer, merged_outlets):      
        wbt.unnest_basins(
    ...
    def split_raster(dem_dir, polygon_dir, split_raster_dir):
        for polygon in polygon_dir:
            if polygon.endswith('.shp'):
                wbt.clip_raster_to_polygon(
                    i = dem_dir + f, 
                    polygons = polygon_dir + polygon, 
                    output = split_raster_dir + f.replace('.shp', '.tif'), 
...
def main(temp_dir, dem_dir, d8_pointer, merged_outlets, individual_watersheds_dir, split_raster_dir):
    watershed = watersheds(temp_dir)    
    for f in os.listdir(dem_dir):
        if f.endswith('.tif'):
            watershed.unnest_basins(f, d8_pointer, merged_outlets)
            watershed.split_polygon(individual_watersheds_dir)
            watershed.split_raster(individual_watersheds_dir, split_raster_dir)
...
if __name__== '__main__':
    ...
    parser.add_argument('d8_pointer', help='Path directory of d8_file')
    parser.add_argument('merged_outlets', help='shapefile with digitised points of lake outlets')
    #parser.add_argument('watershed_dir', help='path to dir where watersheds will be stored')
